...
class EmbeddedSpatialTypesIT
{
    ...
    {
        Stream<Value> randomPoints = ThreadLocalRandom.current().ints( 1_000, 0, 2 ).mapToObj(
                idx -> idx % 2 == 0 ? point( WGS_84_CRS_CODE, randomDouble(), randomDouble() ) : point( CARTESIAN_CRS_CODE, randomDouble(), randomDouble() ) );
    ...
    {
        Stream<List<Value>> randomPointLists = ThreadLocalRandom.current().ints( 1_000, 0, 2 ).mapToObj( EmbeddedSpatialTypesIT::randomPointList );
    ...
    private static List<Value> randomPointList( int index )
    {
        ...
        return IntStream.range( 0, size ).mapToObj( i -> point( srid, randomDouble(), randomDouble() ) ).collect( toList() );
    }
