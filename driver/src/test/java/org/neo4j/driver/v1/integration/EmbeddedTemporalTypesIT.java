...
class EmbeddedTemporalTypesIT
{
    ...
    {
        testReceiveValue( "RETURN time({hour: 23, minute: 19, second: 55, timezone:'-07:00'})", OffsetTime.of( 23, 19, 55, 0, ZoneOffset.ofHours( -7 ) ),
                Value::asOffsetTime );
    }
    ...
    {
        testReceiveValue( "RETURN localtime({hour: 22, minute: 59, second: 10, nanosecond: 999999})", LocalTime.of( 22, 59, 10, 999_999 ), Value::asLocalTime );
    }
    ...
    void shouldReceiveLocalDateTime()
    {
        testReceiveValue( "RETURN localdatetime({year: 1899, month: 3, day: 20, hour: 12, minute: 17, second: 13, nanosecond: 999})",
                LocalDateTime.of( 1899, MARCH, 20, 12, 17, 13, 999 ), Value::asLocalDateTime );
    }
    ...
    void shouldReceiveDateTimeWithZoneOffset()
    {
        ...
        testReceiveValue( "RETURN datetime({year:1984, month:10, day:11, hour:21, minute:30, second:34, timezone:'+03:30'})",
                ZonedDateTime.of( 1984, 10, 11, 21, 30, 34, 0, offset ), Value::asZonedDateTime );
    }
    ...
    void shouldReceiveDateTimeRepresentedWithOffsetDateTime()
    {
        testReceiveValue( "RETURN datetime({year:2121, month:1, day:1, hour:2, minute:2, second:2, timezone:'-07:20'})",
                OffsetDateTime.of( 2121, 1, 1, 2, 2, 2, 0, ZoneOffset.ofHoursMinutes( -7, -20 ) ), Value::asOffsetDateTime );
    }
    ...
    void shouldReceiveDateTimeWithZoneId()
    {
        ...
        testReceiveValue( "RETURN datetime({year:2000, month:1, day:1, hour:9, minute:5, second:1, timezone:'Europe/London'})",
                ZonedDateTime.of( 2000, 1, 1, 9, 5, 1, 0, zoneId ), Value::asZonedDateTime );
    }
    ...
    {
        testReceiveValue( "RETURN duration({months: 13, days: 40, seconds: 12, nanoseconds: 999})", newDuration( 13, 40, 12, 999 ), Value::asIsoDuration );
    }
    ...
    {
        Record record1 = session.run( "CREATE (n:Node {value: $value}) RETURN 42 as v, id(n) as id", singletonMap( "value", value ) ).single();
        assertEquals( 42, record1.get( "v" ).asInt() );
        long id = record1.get( "id" ).asLong();

        Record record2 = session.run( "MATCH (n:Node) WHERE id(n) = $id RETURN n.value", singletonMap( "id", id ) ).single();
